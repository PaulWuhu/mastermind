...
* Inside the front-end structure, we have the basic setup for react with TypeScript using vite. In side the src folder, we have our app. In the assets folder, we have our contexts provider for the app. And we have the rest of the app in the components folders for each page. 
...
* The acl.py file contains a function for calling the Random generator API. We use a different function to call the 3rd party API. So the user will not be able to call the API directly. 
### model.py
...
* This is the score context where we fetch for all the score, and pass the function that fetch the score out. This way, when ever we need to update the score, we can use the function without props grillings.
...
### Board.tsx
![img for Board.tsx](img_for_readme/boardtax.png)
