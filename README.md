...
* This file contains a function for calling the Random generator API. We use a different function to call the 3rd party API. So the user will ot be able to call the API directly. 

![model.py code img](img_for_readme/modelpy.png)
* This file is where we store our model for our database table. Each table would have id(generated by default), username, password, win and loss. Note, very important, the user model is from Models.model class, rather than the django build-in authentication models.
...
They are: 
* get_random_number for getting a random number
* log_in for logging in a user
* signup for create user
* api_get_user for getting a user or deleting a user
* api_get_all_user for get all the user
* api_score for updating the score
